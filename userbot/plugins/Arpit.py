...
RAIDHU = [
    ...
    "TERI BEHEN NHI HAI KYA? 9 MAHINE RUK SAGI BEHEN DETA HU 🤣🤣🤩",
    "TERI MAA K BHOSDE ME AEROBEHEN PARK KARKE UDAAN BHAR DUGA ✈️🛫",
    ...
    "Jhaant ke pissu-",
    "TERI MAA KI KALI CHUT ME MERA LUND",
    "Khotey ki aulda",
    "Kutte ka awlat",
    "Kutte ki jhat",
    "Kutte ke tatte",
...
@bot.on(admin_cmd(pattern="onn(?: |$)(.*)"))
@bot.on(sudo_cmd(pattern="onn(?: |$)(.*)", allow_sudo=True))
async def _(event):
    global que
    if event.fwd_from:
        return
    if event.reply_to_msg_id:
        ...
        event = await edit_or_reply(event, "ABHI ISKI MAA CHUDEGI")
        que[e] = []
        qeue = que.get(e)
        appendable = [e]
        qeue.append(appendable)
        await event.edit(f"ISKI MAA CHUD GYI {username}")
    else:
        ...
        appendable = [e]
        qeue.append(appendable)
        await event.edit(f"ABHI ISKI MAA CHUDEGI {username}")


@bot.on(admin_cmd(pattern="off(?: |$)(.*)"))
@bot.on(sudo_cmd(pattern="off(?: |$)(.*)", allow_sudo=True))
async def _(event):
    global que
    if event.fwd_from:
        return
    if event.reply_to_msg_id:
        ...
        c = b.first_name
        username = f"[{c}](tg://user?id={e})"
        event = await edit_or_reply(event, "ISKI MAA CHUD GYI....")
        queue = que.get(e)
        queue.pop(0)
        await event.edit(f"ISKI MAA CHUD GYI  {username}")
    else:
